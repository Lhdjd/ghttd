...
class bookManager:
    ...
    # 1. 添加图书
    def add_book(self, book_id, name, auther, cla):
        ...
        try:
            ...
            self.conn.commit()
            print("添加成功")
            self.write_log(f"用户{self.current_user['username']} 新增图书：书号{book_id} 书名{name}")
    ...
    def show_all_book(self):
        ...
        self.write_log(f"用户{self.current_user['username']} 查询所有图书")
    ...
    def search_book_by_id(self, book_id):
        ...
        if res:
            ...
            print(f"分类：{res['cla']}")
            print(f"状态：{res['sta']}")
            self.write_log(f"用户{self.current_user['username']} 查询图书：书号{book_id}")
        else:
            print("未查询到该图书信息！")
    ...
    def update_book_info(self, book_id, new_name, new_auther, new_cla):
        try:
            ...
            if self.cursor.rowcount > 0:
                print("基础信息修改成功")
                self.write_log(f"用户{self.current_user['username']} 修改图书：书号{book_id}")
            else:
                print("未找到该书")
        except Exception as e:
            self.conn.rollback()
    ...
    def delete_book(self, book_id):
        try:
            ...
            if self.cursor.rowcount > 0:
                print("图书信息已删除")
                self.write_log(f"用户{self.current_user['username']} 删除图书：书号{book_id}")
            else:
                print("未找到该图书")
        except:
            self.conn.rollback()
    ...
    def borrow_book(self, book_id):
        try:
            ...
            if res["sta"] == "可借阅":
                ...
                self.conn.commit()
                print("借阅成功")
                self.write_log(f"用户{self.current_user['username']} 借阅图书：书号{book_id}")
            else:
                print("图书已借出，无法借阅")
        except:
            self.conn.rollback()
    ...
    def back_book(self, book_id):
        try:
            ...
            if res["sta"] == "已借出":
                ...
                self.conn.commit()
                print("归还成功")
                self.write_log(f"用户{self.current_user['username']} 归还图书：书号{book_id}")
            else:
                print("图书未借出，无需归还")
        except:
            self.conn.rollback()
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("系统退出！")
            bm.write_log(f"用户{bm.current_user['username']} 退出系统")
            bm.close()
            break
